...
// Accepts only canonical UUID v4 — rejects anything else, preventing header injection.
const UUID_V4_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i

export function validateDeviceId(id) {
  return typeof id === 'string' && UUID_V4_RE.test(id) ? id : null
}

// ── Per-device pin rate limiter ───────────────────────────────────────────────
// Separate from the general AI rate limiter. Allows 1 pin drop per device per
// PIN_WINDOW_MS window so a single user can't flood the community map.
const pinRateStore = new Map()
const PIN_WINDOW_MS = 60_000    // 1 minute
const PIN_RATE_MAX  = 1         // 1 pin per window per device

export function checkPinRateLimit(deviceId) {
  const now = Date.now()
  const record = pinRateStore.get(deviceId) || { count: 0, resetAt: now + PIN_WINDOW_MS }

  if (now > record.resetAt) {
    record.count  = 1
    record.resetAt = now + PIN_WINDOW_MS
  } else {
    record.count++
  }

  pinRateStore.set(deviceId, record)

  // Prune expired entries so the map doesn't grow unbounded
  if (pinRateStore.size > 5_000) {
    for (const [key, val] of pinRateStore) {
      if (now > val.resetAt) pinRateStore.delete(key)
    }
  }

  return record.count <= PIN_RATE_MAX
}
