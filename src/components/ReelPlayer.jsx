...
export default function ReelPlayer({ data, onClose }) {
  ...
  useEffect(() => {
    if (!isPlaying) return
    const timers = [
      setTimeout(() => setActiveBg(1), 5000),
      setTimeout(() => setActiveBg(2), 14000),
      setTimeout(() => setActiveBg(0), 22000),
      ...
      const renderFrame = () => {
        ...
        if (t >= 5 && t < 14.5) {
          ...
          if (segT > 4) {
            ...
            ctx.fillStyle = 'white'
            ctx.fillText(`${topLocations[0]?.area || 'Library'} leads with ${topLocations[0]?.total || 0} pins`, lx, h - 100)
          }
          ctx.globalAlpha = 1
        }
